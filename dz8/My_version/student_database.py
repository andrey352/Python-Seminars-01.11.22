...
def set_student(data_list):
    """Записывает данные ученика в словарь. Пример записи: {'Иванов': [['Иван', '1A'], {'Инф':['5','5','5']}]}"""
    student_db[data_list[0]] = data_list[1:], {}


def set_rating(last_name, lesson, rating):                         # Добавление оценок
...
def get_student(last_name_student):                               # вывод данных ученика
    if student_db.get(last_name_student) is None:
        print('Ученик с фамилией {last_name_student} не найден')
    else:
